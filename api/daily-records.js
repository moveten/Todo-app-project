    ...
    if (req.method === 'POST') {
      ...
      const cleanEntries = Array.isArray(entries)
        ? entries.slice(0, 30).map((e) => ({
            ...
            type: String((e && e.type) || 'etc').slice(0, 20),
            score: [1, 2, 3, 4, 5].includes(Number(e && e.score)) ? Number(e.score) : null,
            fields: Object.fromEntries(
              Object.entries((e && e.fields) || {})
                .filter(([k, v]) => typeof v === 'string' && v.trim())
                .map(([k, v]) => [String(k).slice(0, 20), v.slice(0, 4000)])
            ),
            tags: cleanTags(e && e.tags),
          }))
        : null;
      const entJson = cleanEntries ? JSON.stringify(cleanEntries) : null;
      ...
      const rows = await sql`
        ...
        SET mood = ${mood || null},
            ...
            categories = CASE WHEN ${entJson}::jsonb IS NOT NULL THEN daily_records.categories ELSE ${catJson}::jsonb END,
            entries = COALESCE(${entJson}::jsonb, daily_records.entries),
            reflection = ${refJson},
            routines = ${routJson},
            advice = ${advice || null},
            plan = ${plan || null},
